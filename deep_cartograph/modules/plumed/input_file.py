...
def track_features(configuration: Dict, topology_path: str, colvars_path: str, output_folder: str) -> Tuple[str, str]:
    ...
    if configuration['features'].get('distance_groups', {}) != {}:
        ...
        for distance_group_name in distance_group_names:    
            ...
            distance_names, atomic_definitions = plumed.utils.get_distance_labels(plumed_topology_path, distance_group, distance_group_name)
    ...
    if configuration['features'].get('dihedral_groups', {}) != {}:
        ...
        for dihedral_group_name in dihedral_group_names:
            ...
            dihedral_names, atomic_definitions = plumed.utils.get_dihedral_labels(plumed_topology_path, dihedral_group, dihedral_group_name)
            
            if dihedral_group.get('periodic_encoding', True):
                logger.info(f"Found {2*len(atomic_definitions)} features for {dihedral_group_name}, (sin and cos of each of the {len(atomic_definitions)} torsion angles)") # Each dihedral has 2 features (sin and cos)
            else:
